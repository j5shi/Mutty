...
void
win_set_title(struct term *term, char *title_)
{
    char *title = basename(title_);
...
update_glass(void)
{
    if (pDwmExtendFrameIntoClientArea)
    {
        ...
        pDwmExtendFrameIntoClientArea(wnd, &(MARGINS)
        {
            enabled ? -1 : 0, 0, 0, 0
        });
    }
...
when IDM_FLIPSCREEN:
                    term_flip_screen(term);
when IDM_OPTIONS:
                    win_open_config();
when IDM_NEW:
                    child_fork(term->child, main_argc, main_argv);
when IDM_COPYTITLE:
                    win_copy_title();
when IDM_NEWTAB:
                    win_tab_create();
...
when WM_MOUSEMOVE:
            win_mouse_move(false, lp);
when WM_NCMOUSEMOVE:
            win_mouse_move(true, lp);
when WM_MOUSEWHEEL:
...
main(int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        char *longopt = argv[optind - 1], *shortopt = (char[])
        {'-', optopt, 0
        };
...
when 'c':
                load_config(optarg);
when 'h':
                set_arg_option("Hold", optarg);
when 'i':
                set_arg_option("Icon", optarg);
when 'l':
                set_arg_option("Log", optarg);
when 'o':
                parse_arg_option(optarg);
when 'p':
...
when 'u':
                cfg.utmp = true;
when 'w':
                set_arg_option("Window", optarg);
when 'b':
                tablist[current_tab_size] = optarg;
                current_tab_size++;
when 'C':
                set_arg_option("Class", optarg);
when 'H':
