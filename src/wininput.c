...
get_mods(void)
{
    inline bool is_key_down(uchar vk)
    {
...
win_key_down(WPARAM wp, LPARAM lp)
{
    ...
    if (key == VK_PROCESSKEY)
    {
        TranslateMessage(
            &(MSG)
        {
            .hwnd = wnd, .message = WM_KEYDOWN, .wParam = wp, .lParam = lp
        }
        );
        return 1;
    }
    ...
    if (!active_term->shortcut_override)
    {
        ...
        if (cfg.zoom_shortcuts && mods == MDK_CTRL)
        {
            ...
            switch (key)
            {
when VK_OEM_PLUS or VK_ADD:
                    zoom = 1;
when VK_OEM_MINUS or VK_SUBTRACT:
                    zoom = -1;
when '0' or VK_NUMPAD0:
                    zoom = 0;
otherwise:
                    goto not_zoom;
            }
...
when VK_F4:
                        send_syscommand(SC_CLOSE);
when VK_F8:
                        send_syscommand(IDM_RESET);
when VK_F10:
                        send_syscommand(IDM_DEFSIZE);
when VK_F11:
                        send_syscommand(IDM_FULLSCREEN);
when VK_F12:
                        send_syscommand(IDM_FLIPSCREEN);
                }
            }
...
not_scroll:
    ...
    void mod_csi(char c)
    {
        len = sprintf(buf, "\e[1;%c%c", mods + '1', c);
    }
    void mod_ss3(char c)
    {
        mods ? mod_csi(c) : ss3(c);
    }
...
when VK_NEXT:
            edit_key(6, '3');
when VK_HOME:
            active_term->vt220_keys ? edit_key(1, '7') : cursor_key('H', '7');
when VK_END:
            active_term->vt220_keys ? edit_key(4, '1') : cursor_key('F', '1');
when VK_UP:
            cursor_key('A', '8');
when VK_DOWN:
            cursor_key('B', '2');
...
win_key_up(WPARAM wp, LPARAM unused(lp))
{
    ...
    if (alt_state > ALT_ALONE && alt_code)
    {
        ...
        {
            xchar xc = alt_code;
            child_sendw(active_term->child, (wchar[])
            {
                high_surrogate(xc), low_surrogate(xc)
            }, 2);
        }
    }
