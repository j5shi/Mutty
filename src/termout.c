                ...
                for (int i = 0; i < term->rows; i++)
            {
                ...
                for (int j = 0; j < term->cols; j++)
                {
                    line->chars[j] =
                        (termchar)
                    {
                        .cc_next = 0, .chr = 'E', .attr = CATTR_DEFAULT
                    };
                }
...
when 5:
                attr.attr |= ATTR_BLINK;
when 7:
                attr.attr |= ATTR_REVERSE;
when 8:
                attr.attr |= ATTR_INVISIBLE;
when 9:
                attr.attr |= ATTR_STRIKEOUT;
when 10 ... 12:
...
when 11:
            child_write(term->child, win_is_iconic() ? "\e[1t" : "\e[2t", 4);
when 13:
            {
                int x, y;
                win_get_pos(&x, &y);
...
when 18:
            child_printf(term->child, "\e[8;%d;%dt", term->rows, term->cols);
when 19:
            {
                int rows, cols;
                win_get_screen_chars(&rows, &cols);
...
when - 1:
            do_dcs(term);
when 0 or 2:
            win_set_title(term, s);  // ignore icon title
when 4:
            do_colour_osc(term, 0);
when 10:
            do_colour_osc(term, FG_COLOUR_I);
when 11:
            do_colour_osc(term, BG_COLOUR_I);
when 12:
            do_colour_osc(term, CURSOR_COLOUR_I);
